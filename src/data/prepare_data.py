...
def read_sample(slice: bool = False) -> DataFrame:
    # df = pd.read_csv('../../data/raw/reviews.csv')
    # df['rating'] = df['rating'].astype(dtype='int64')
    df = pd.read_json('../../data/raw/newsgroups.json')
    df.drop(columns=['target', 'target_names'], inplace=True)
    if slice:
        df = df[:100]
...
def preprocess_data(df: DataFrame, save: bool = False):
    ...
    if save:
        with open("../../data/interim/data_lemmatized.pkl", "wb") as output_file:
            pickle.dump(data_lemmatized, output_file)
        logging.info('Model saved to artifact data_lemmatized.pkl')

    return data_lemmatized


def main():
    df = read_sample(False)
